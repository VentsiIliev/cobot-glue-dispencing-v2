...
class LaserTracker(object):
    ...
    def __init__(self, cam_width=1280, cam_height=720, hue_min=20, hue_max=160,
        ...
        * ``display_thresholds`` -- if True, additional windows will display
        ...
        # Update the path to the calibration model file
        calibration_model_path = os.path.join(os.path.dirname(__file__), 'calibration_model.pkl')
        with open(calibration_model_path, 'rb') as file:
            self.poly_func = pickle.load(file)
    ...
    #
    #     # Ensure self.trail and frame have the same size and number of channels
    #     if self.trail.shape != frame.shape:
    #         self.trail = cv2.resize(self.trail, (frame.shape[1], frame.shape[0]))
    #
    #     cv2.add(self.trail, frame, frame)
    #
    #     if center is not None and self.previous_position is not None:
    #
    #         self.centerMM_Y = center[1] / self.ppmY
    #
    #         self.diff = abs(self.centerMM_Y - (363 / self.ppmY))
    #
    ...
    def track(self, frame, mask):
        ...
        if len(contours) > 0:
            ...
            if moments["m00"] > 0:
                ...
                center = int(x), int(y)
            print("center ",center)
            # Ensure the laser is around the center of the image (half the width)
            image_center_x = frame.shape[1] // 2
            if abs(center[0] - image_center_x) < 50:  # You can adjust the threshold as needed
                if radius > 10:
                    cv2.circle(frame, (int(x), int(y)), int(radius), (0, 255, 255), 2)
                    cv2.circle(frame, center, 5, (0, 0, 255), -1)
                    if self.previous_position:
        ...
        if center is not None and self.previous_position is not None:
            # Assuming self.ppmY is used for converting the Y coordinate to millimeters.
            self.centerMM_Y = center[1] / self.ppmY
            self.diff = abs(self.centerMM_Y - (368 / self.ppmY))
    ...
    def run(self, frameParam=None):

        if frameParam is not None:
            hsv_image = self.detect(frameParam)
            estimated_height = None
            if self.diff is not None:
                finalDiff = self.diff
                print("finalDiff ",finalDiff)
                estimated_height = self.poly_func(finalDiff)
                cv2.putText(frameParam, f"Height: {finalDiff:.2f} pixels", (100, 200), cv2.FONT_HERSHEY_SIMPLEX, 2,
                            (255, 0, 0),
                            10, cv2.LINE_AA)
                cv2.putText(frameParam, f"Height: {estimated_height:.2f} mm", (100, 400), cv2.FONT_HERSHEY_SIMPLEX, 2,
                            (255, 0, 0),
                            10, cv2.LINE_AA)
            # self.display(hsv_image, frameParam)
            self.handle_quit()
            # return self.diff
...
#         diff = tracker.run(frame)
#         if diff is not None:
#             cv2.putText(frame, f"Height: {diff:.2f} mm", (100, 200), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0),
#                         10, cv2.LINE_AA)
