...
class ConfigChangeCommand(Command):
    ...
    def execute(self):
        ...
        self.controller.save_config_to_file(self.new_config)

    def undo(self):
        self.controller.apply_config_to_ui(self.old_config)
        self.controller.save_config_to_file(self.old_config)
...
class RobotConfigController:
    ...
    def on_velocity_changed(self, group_name, value):
        ...
        if not self.is_loading:
            ...
            if group_name not in new_config["MOVEMENT_GROUPS"]:
                new_config["MOVEMENT_GROUPS"][group_name] = {}
    ...
    def apply_config_to_ui(self, config):
        ...
        self.ui.ip_edit.setText(config.get("ROBOT_IP", "192.168.58.2"))
        self.ui.tool_edit.setValue(config.get("ROBOT_TOOL", 0))
        self.ui.user_edit.setValue(config.get("ROBOT_USER", 0))

        movement_groups = config.get("MOVEMENT_GROUPS", {})

        for group_name, group_data in movement_groups.items():
            if group_name in self.ui.position_lists:
                widget = self.ui.position_lists[group_name]

                if isinstance(widget, QLineEdit):
                    position = group_data.get("position", "")
                    widget.setText(position)
                elif isinstance(widget, QListWidget):
                    widget.clear()
            ...
            if group_name in self.ui.velocity_acceleration_widgets:
                ...
                widgets["velocity"].setValue(group_data.get("velocity", 0))
                widgets["acceleration"].setValue(group_data.get("acceleration", 0))
    ...
    def get_current_config(self):
        ...
        for group_name, widget in self.ui.position_lists.items():
            ...
            config["MOVEMENT_GROUPS"][group_name] = group_data

        for group_name, widgets in self.ui.velocity_acceleration_widgets.items():
            if group_name not in config["MOVEMENT_GROUPS"]:
                config["MOVEMENT_GROUPS"][group_name] = {}

            config["MOVEMENT_GROUPS"][group_name]["velocity"] = widgets["velocity"].value()
            config["MOVEMENT_GROUPS"][group_name]["acceleration"] = widgets["acceleration"].value()
...
class RobotConfigUI(QWidget):
    def __init__(self):
        ...
        self.controller = RobotConfigController(self)
